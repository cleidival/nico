 ...
 <div id="modalResultadoRemarcacao" class="modal">
   <div class="modal-dialog">
	...
	$("#btnAdicionarConfiguracoes").click(function(){
		...
		$.ajax({
			url: "processa.aspx",
	        data: {"Pagina":"ConfiguracoesPrecos","Acao":"Adicionar", "Classe":classe, "Subclasse":subclasse, "Grupo":grupo, "Familia":familia, "Tabela1":tabela1, "Tabela2":tabela2, "Percentual":percentual, "Operacao":operacao},
        	success:function(data){  
        	    $(".status").html("Dados gravados...").addClass("success").fadeIn();
			    $(".lista-configuracoesPrecos tbody").append("<tr><td><input type='checkbox' name='codigo[]' class='codigo[]' value=''></td><td>"+(content_classe!="Selecione..."?content_classe.split(':')[1]:"Todos")+"</td><td>"+(content_subclasse!="Selecione..."?content_subclasse.split(':')[1]:"Todos")+"</td><td>"+(content_grupo!="Selecione..."?content_grupo.split(':')[1]:"Todos")+"</td><td>"+(content_familia!="Selecione..."?content_familia.split(':')[1]:"Todos")+"</td><td>"+content_tabela1+"</td><td>"+content_tabela2+"</td><td>"+percentual+"</td><td>"+operacao+"</td></tr>").css("color","dark green").fadeIn();
	        },
			error:function(data){
